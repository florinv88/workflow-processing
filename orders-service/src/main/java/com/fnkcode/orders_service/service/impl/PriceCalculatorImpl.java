...
import com.fnkcode.orders_service.service.PriceCalculator;
...
public class PriceCalculatorImpl implements PriceCalculator {
    ...
    public PriceSummary calculate(Order order) {
        ...
        return switch (order.orderItem().product()){
            case Product.Single single -> this.toPriceSummary(single.price(), single.price(), quantity, state);
            case Product.Bundle bundle -> this.toPriceSummary(bundle.originalPrice(), bundle.discountedPrice(), quantity, state);
        };
    }
